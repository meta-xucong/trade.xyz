...
- 提供当前 Dashboard 市场的 `Cancel All / Stop Strategies` 动作。该动作必须由后端重新拉取
  交易所 open orders 后执行，不得信任前端缓存；它会撤销当前市场所有 exchange-open
  委托，并把关联 Fib 策略标记为 stopped / `auto_loop=false`，防止撤单后策略继续自动挂单。
- 单个 Fib 策略的停止按钮仍必须调用 Fib 模块内部 API；Manual maker resting orders 没有策略
  实例时，以 `source_module=manual` 作为普通挂单显示和撤销。
