...
- 当前模式：dry-run / live。
- 总览市场切换：`xyz_perp` / `hl_perp` / `cash_perp` / `spot`。
...
- `cash_perp`：使用 `clearinghouseState` + `dex = "cash"`。它是 HIP-3
  `cash` perp DEX，不属于 trade[XYZ] 的 `xyz` DEX；前端标签应显示为
  `Cash Perps`，避免和 XYZ Perps 混淆。
...
- 通过内部 API 聚合执行模块的 open-order 快照与策略模块的实例状态，不得由前端直接推断
  策略状态。
- 按市场分组展示，避免 `xyz_perp` / `hl_perp` / `cash_perp` / `spot`
  的挂单混在一起。
...
完整后端规格见 [Smart Money Copy Development Spec](smart-money-copy-development.md)。前端只展示
状态和提交配置/控制命令，不参与 leader 行为识别、冲突裁决、买量计算或下单。
...
- 最近风控拒绝。
- 本账户和 leader 的仓位映射状态。
- leader watcher stream health、最后消息时间、重连次数和 gap 状态。
- 同 symbol 多 leader 冲突窗口、方向分数、最终裁决或跳过原因。
- pending exposure、post-close reentry guard 和 copy ledger 状态。
...
- 调整 symbol limit。
- 预览跟单 sizing：当前 MVP 使用“跟单比例 -> 本金上限 -> 杠杆”的顺序计算。单交易对
  默认本金上限为 10 USDC，杠杆倍率最多 5x；低于本金上限时按跟单比例得到的本金跟随，
  高于上限时先截断到本金上限，再乘杠杆得到最终信号 notional。
- 暂停跟单策略。
