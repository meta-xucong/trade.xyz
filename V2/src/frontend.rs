...
use crate::{
    ...
    config::{
        AccountConfig, AppConfig, MARKET_CASH_PERP, MARKET_HL_PERP, MARKET_SPOT, MARKET_XYZ_PERP,
        load_config, normalize_market_id, normalize_market_list, save_config, supported_market_ids,
    },
    domain::{
    ...
    trading::{
        ...
        ensure_live_account_address, exchange_min_effective_detail, execute_cancel_by_cloid,
        execute_cancel_open_order, execute_fast_protective_exit_arm, execute_fast_signed_order,
        ...
        minimum_requested_notional_for_effective_min, reduce_only_position_available,
        reduce_only_position_detail, signed_close_exempt_from_opening_rules,
        summarize_account_readiness_state, tif_for_execution_mode,
        validate_exchange_min_order_notional, validate_live_action_gates,
...
const COPY_LIVE_SOAK_PAUSE_PATH: &str = ".codex-longrun/copy-live-soak-paused.flag";
...
async fn fib_instances(
    ...
    let result = (|| {
        let config = state.config_snapshot()?;
        let fib_history = read_fib_instance_history_entries(FIB_HISTORY_RESPONSE_LIMIT)?;
...
async fn copy_shadow_history() -> Json<ApiResult<CopyShadowHistoryResponse>> {
    Json(ApiResult::from_result(build_copy_shadow_history_response(
        COPY_SHADOW_HISTORY_RESPONSE_LIMIT,
    )))
}

async fn copy_live_soak_status() -> Json<ApiResult<CopyLiveSoakStatusResponse>> {
    Json(ApiResult::from_result(
        build_copy_live_soak_status_response(),
    ))
}
...
async fn smart_money_preview(
    ...
    let audit_details = json!({
        ...
        "principal_cap_usd": payload.principal_cap_usd,
        "leverage": payload.leverage,
        "legacy_max_signal_notional_usd": payload.max_signal_notional_usd,
        "reduce_only": payload.reduce_only,
    });
    let audit_coin = Some(payload.coin.clone());
    let result = state.config_snapshot().and_then(|base_config| {
        ...
        let mut payload = payload;
        payload.coin = canonical_coin;
        let sizing = payload.sizing_preview()?;
        payload.try_into_signal(&config, &sizing).map(|signal| {
...
async fn execute_fib_residual_cleanup_if_needed(
    ...
    let result = execute_fast_signed_order(
        config.clone(),
        options,
        config.app.dry_run,
        vault_password,
...
fn build_copy_shadow_history_response(limit: usize) -> Result<CopyShadowHistoryResponse> {
    Ok(CopyShadowHistoryResponse {
        entries: read_recent_copy_shadow_history_entries_from_active_paths(limit)?,
        fetched_at_ms: now_ms(),
    })
}

#[cfg(test)]
fn build_copy_shadow_history_response_from_path(
    path: &Path,
    limit: usize,
) -> Result<CopyShadowHistoryResponse> {
    Ok(CopyShadowHistoryResponse {
        entries: read_recent_copy_shadow_history_entries(path, limit)?,
        fetched_at_ms: now_ms(),
    })
}
...
fn read_copy_ui_settings() -> Result<CopyUiSettings> {
    ...
    let raw_account_id = settings.account_id.trim().to_string();
    let account_ids = normalize_copy_account_ids(String::new(), settings.account_ids);
    let account_id = if raw_account_id.is_empty() {
        account_ids
            .first()
            .cloned()
    ...
    settings.account_ids = normalize_copy_account_ids(account_id, account_ids);
    Ok(settings)
}

fn normalize_copy_account_ids(primary: String, raw: Vec<String>) -> Vec<String> {
    ...
    if !primary.is_empty() {
        account_ids.retain(|account_id| account_id != &primary);
        account_ids.insert(0, primary);
    }
    account_ids
}
...
fn start_copy_live_soak_from_frontend(
    ...
    let config = state.config_snapshot()?;
    let missing_accounts = settings
        .account_ids
        .iter()
        .filter(|account_id| config.account(account_id).is_none())
        .cloned()
        .collect::<Vec<_>>();
    anyhow::ensure!(
        missing_accounts.is_empty(),
        "copy account(s) are not configured: {}",
    ...
    clear_copy_live_soak_pause_flag()?;
...
fn copy_live_soak_script_path() -> PathBuf {
    let versioned = PathBuf::from("V2")
        .join("scripts")
        .join("run-persistent-live-soak.ps1");
    if versioned.exists() {
        return versioned;
    }
    PathBuf::from(COPY_LIVE_SOAK_DIR).join("run-persistent-live-soak.ps1")
}

fn write_copy_live_soak_pause_flag() -> Result<()> {
    fs::create_dir_all(COPY_LIVE_SOAK_DIR)
        .with_context(|| format!("failed to create {}", COPY_LIVE_SOAK_DIR))?;
    let body = json!({
        "paused_at_ms": now_ms(),
        "reason": "frontend_cancel_copy",
    });
    write_json_pretty(Path::new(COPY_LIVE_SOAK_PAUSE_PATH), &body)
}

fn copy_live_soak_pause_flag_exists_in_dir(dir: &Path) -> bool {
    dir.join("copy-live-soak-paused.flag").exists()
}

fn clear_copy_live_soak_pause_flag() -> Result<()> {
    let path = Path::new(COPY_LIVE_SOAK_PAUSE_PATH);
    if path.exists() {
...
fn stop_copy_live_soak_from_frontend() -> Result<CopyLiveSoakActionResponse> {
    ...
    if !before.running {
        return Ok(CopyLiveSoakActionResponse {
            ...
            status: before,
            message: "copy live soak is paused; no background copy process is running".to_string(),
            pid: None,
        });
    }
...
fn read_recent_copy_shadow_history_entries_from_active_paths(
    ...
    let mut paths = vec![PathBuf::from(COPY_SHADOW_HISTORY_PATH)];
...
fn read_recent_copy_shadow_history_entries_from_paths(
    ...
    for path in paths {
        entries.extend(read_recent_copy_shadow_history_entries(&path, limit)?);
    }
    entries.sort_by_key(|entry| std::cmp::Reverse(entry.occurred_at_ms));
    let mut seen = HashSet::<String>::new();
    entries.retain(|entry| {
        let key = format!(
            ...
            entry.status,
            entry.leader_address.to_ascii_lowercase(),
            entry.coin,
            entry.action_event_id
        );
        seen.insert(key)
    });
    entries.truncate(limit);
...
fn active_copy_live_soak_shadow_history_path() -> Result<Option<PathBuf>> {
    let dir = copy_live_soak_dir();
    if !dir.exists() {
        return Ok(None);
    }
    ...
    for line in raw.lines() {
        let Some(path_text) = line.split(" shadow=").nth(1) else {
            continue;
        };
        let path = path_text
            .split_whitespace()
    ...
    latest_matching_file(&dir, "-shadow.jsonl")
}

fn build_copy_summary_response_from_entries(
    entries: Vec<CopyShadowHistoryEntry>,
    ...
    let configured_account_ids = copy_settings
    ...
    for entry in &summary_entries {
        if !entry.leader_address.trim().is_empty() {
            ...
            let meta = leader_event_meta
                .entry(key)
                .or_insert_with(|| CopyLeaderEventMeta {
                    leader_id: entry.leader_id.clone(),
                    signal_count: 0,
                    last_signal_at_ms: None,
                });
            if meta.leader_id.trim().is_empty() && !entry.leader_id.trim().is_empty() {
                meta.leader_id = entry.leader_id.clone();
            }
            meta.signal_count += 1;
            meta.last_signal_at_ms = meta
                .last_signal_at_ms
                .map(|current| current.max(entry.occurred_at_ms))
                .or(Some(entry.occurred_at_ms));
        }
        latest_signal_at_ms = latest_signal_at_ms
            .map(|current: u64| current.max(entry.occurred_at_ms))
            .or(Some(entry.occurred_at_ms));
    ...
    let live_running = live_soak
        ...
        .unwrap_or(false);
    let live_round = live_soak.as_ref().and_then(|status| status.latest_round);
    let submitted_reports = live_soak
        .as_ref()
        .and_then(|status| status.submitted_reports)
        .unwrap_or(0);
    let order_evidence = live_soak
        .as_ref()
    ...
    let mut local_summary = live_accounts
        ...
        .or_else(|| {
            (live_realized_pnl_usd.is_some()
                ...
                || live_total_fees_usd.is_some())
            .then(|| CopyPnlAccountSummary {
                id: "local".to_string(),
    ...
    if pnl_summary.submitted_notional_usd > 0.0 {
        copied_notional_usd = copied_notional_usd.max(pnl_summary.submitted_notional_usd);
    } else if let Some(local) = local_summary.as_ref() {
        copied_notional_usd =
            copied_notional_usd.max(local.notional_usd.unwrap_or_default().max(0.0));
    }
    ...
    for account in live_accounts.targets.iter().chain(
        ...
        if let Some(meta) = leader_event_meta.get(&key) {
            if leader.id.trim().is_empty() && !meta.leader_id.trim().is_empty() {
                leader.id = meta.leader_id.clone();
            }
            leader.signal_count = meta.signal_count;
            leader.last_signal_at_ms = meta.last_signal_at_ms;
        }
    ...
    let target_realized_pnl_usd = copy_sum_optional(
        leader_summaries
            .iter()
            .map(|leader| leader.realized_pnl_usd),
    );
    let target_unrealized_pnl_usd = copy_sum_optional(
        leader_summaries
            .iter()
            .map(|leader| leader.unrealized_pnl_usd),
    );
    let target_total_pnl_usd =
        copy_sum_optional([target_realized_pnl_usd, target_unrealized_pnl_usd]);
    let target_position_value_usd = copy_sum_optional(
        leader_summaries
            .iter()
            .map(|leader| leader.position_value_usd),
    );
    let target_open_position_count = leader_summaries
        .iter()
...
struct CopyLiveAccountSummaries {
    ...
    locals: Vec<CopyPnlAccountSummary>,
...
async fn read_copy_live_account_summaries(
    ...
    let local_account_ids = settings
        .map(|settings| settings.account_ids.clone())
...
fn aggregate_copy_local_account_summaries(
    accounts: &[CopyPnlAccountSummary],
) -> Option<CopyPnlAccountSummary> {
    if accounts.is_empty() {
        return None;
    }
    let mut positions = Vec::new();
    for account in accounts {
        positions.extend(account.positions.iter().cloned());
    }
    let realized_pnl_usd =
        copy_sum_optional(accounts.iter().map(|account| account.realized_pnl_usd));
    let unrealized_pnl_usd =
        copy_sum_optional(accounts.iter().map(|account| account.unrealized_pnl_usd));
    let fees_usd = copy_sum_optional(accounts.iter().map(|account| account.fees_usd));
    ...
    Some(CopyPnlAccountSummary {
        ...
        unrealized_pnl_usd,
        total_pnl_usd: copy_sum_optional([realized_pnl_usd, unrealized_pnl_usd]),
        fees_usd,
        notional_usd,
        position_value_usd,
        open_position_count: positions.len(),
        positions,
        signal_count: accounts.iter().map(|account| account.signal_count).sum(),
        last_signal_at_ms: accounts
            .iter()
            .filter_map(|account| account.last_signal_at_ms)
            .max(),
        ..CopyPnlAccountSummary::default()
    })
}
...
struct CopyLeaderEventMeta {
    leader_id: String,
    signal_count: u64,
    last_signal_at_ms: Option<u64>,
}

#[derive(Debug, Default)]
struct CopyLiveSoakPnlSummary {
    submitted_reports: u64,
    order_evidence: u64,
    submitted_notional_usd: f64,
    matching_fill_count: u64,
    closed_pnl_usd: f64,
    fees_usd: f64,
}

impl CopyLiveSoakPnlSummary {
    ...
    fn total_fees_usd(&self) -> Option<f64> {
        (self.matching_fill_count > 0).then(|| normalize_display_usd(self.fees_usd))
    }

    fn status_text(&self) -> String {
        if self.matching_fill_count == 0 {
            return "copy live fill evidence not found yet; PnL will appear after submitted orders have matching fills".to_string();
        }
        format!(
            "copy live PnL from {} matching fill(s): closedPnl {:.6} USD, fees {:.6} USD; unrealized copy attribution is not yet connected to live position marks",
            self.matching_fill_count,
            normalize_display_usd(self.closed_pnl_usd),
            normalize_display_usd(self.fees_usd)
        )
    }
}
...
fn read_copy_live_soak_pnl_summary(
    ...
    for entry in fs::read_dir(dir).with_context(|| format!("failed to read {}", dir.display()))? {
        ...
        let Some(submit) = report.get("persistent_live_submit") else {
            continue;
        };
        if let Some(items) = submit.get("submitted_reports").and_then(Value::as_array) {
            summary.submitted_reports += items
                .iter()
                .filter(|item| copy_json_live_submitted_report(item))
                .count() as u64;
            summary.submitted_notional_usd += items
                .iter()
                .filter(|item| copy_json_live_submitted_report(item))
                .filter_map(|item| copy_json_number_or_string(item, "notional_usd"))
                .map(|notional| notional.max(0.0))
                .sum::<f64>();
        }
        summary.order_evidence += submit
            .get("order_evidence")
            .and_then(Value::as_array)
            .map(|items| items.len() as u64)
            .unwrap_or(0);
        let Some(evidence_items) = submit.get("order_evidence").and_then(Value::as_array) else {
            continue;
        };
        for evidence in evidence_items {
            let Some(fills) = evidence.get("matching_fills").and_then(Value::as_array) else {
                continue;
            };
            for fill in fills {
                let key = copy_fill_identity(fill);
                if !seen_fills.insert(key) {
                    continue;
                }
                summary.matching_fill_count += 1;
                summary.closed_pnl_usd += copy_json_number_or_string(fill, "closedPnl")
                    .or_else(|| copy_json_number_or_string(fill, "closed_pnl"))
                    .unwrap_or(0.0);
                summary.fees_usd += copy_json_number_or_string(fill, "fee").unwrap_or(0.0).abs();
            }
        }
    }
    Ok(summary)
...
fn read_latest_copy_pnl_report_summary(dir: &Path) -> Result<CopyPnlReportSummary> {
    if !dir.exists() {
        return Ok(CopyPnlReportSummary::default());
    }
    let Some(path) = latest_copy_pnl_report_file(dir)? else {
        return Ok(CopyPnlReportSummary::default());
    };
    let raw = fs::read_to_string(&path)
        .with_context(|| format!("failed to read copy PnL report {}", path.display()))?;
    let report: Value = serde_json::from_str(&raw)
        .with_context(|| format!("failed to parse copy PnL report {}", path.display()))?;
    let accounts = report
        .get("accounts")
        .and_then(Value::as_array)
        .map(|items| {
            items
                .iter()
                .filter_map(copy_pnl_account_summary_from_json)
                .collect::<Vec<_>>()
        })
        .unwrap_or_default();
...
fn latest_copy_pnl_report_file(dir: &Path) -> Result<Option<PathBuf>> {
    ...
    for entry in fs::read_dir(dir).with_context(|| format!("failed to read {}", dir.display()))? {
        ...
        let name = path
            ...
            .unwrap_or("");
        if !(name.starts_with("pnl-since-") && name.ends_with(".json")) {
            continue;
        }
        let modified = file_modified_at_ms(&path).unwrap_or(0);
        if latest
            .as_ref()
...
fn copy_pnl_account_summary_from_json(value: &Value) -> Option<CopyPnlAccountSummary> {
    let id = value
        .get("id")
    ...
    let address = value
        ...
        .unwrap_or("")
        .to_ascii_lowercase();
    if id.trim().is_empty() && kind.trim().is_empty() && address.trim().is_empty() {
        return None;
    }
    let fill_summary = value
        .get("fillSummary")
        .or_else(|| value.get("fill_summary"));
    let position_summary = value
        .get("positionSummary")
        .or_else(|| value.get("position_summary"));
    let closed_pnl_usd = fill_summary
        .and_then(|summary| copy_json_number_or_string(summary, "closedPnl"))
        .or_else(|| {
            fill_summary.and_then(|summary| copy_json_number_or_string(summary, "closed_pnl"))
        })
        .map(normalize_display_usd);
    let fees_usd = fill_summary
        .and_then(|summary| copy_json_number_or_string(summary, "fees"))
        .map(|value| normalize_display_usd(value.abs()));
    let realized_pnl_usd = fill_summary
        .and_then(|summary| copy_json_number_or_string(summary, "netRealized"))
        .or_else(|| {
            fill_summary.and_then(|summary| copy_json_number_or_string(summary, "net_realized"))
        })
        .or_else(|| Some(closed_pnl_usd.unwrap_or(0.0) - fees_usd.unwrap_or(0.0)))
        .map(normalize_display_usd);
    let unrealized_pnl_usd = position_summary
        .and_then(|summary| copy_json_number_or_string(summary, "unrealizedPnl"))
        .or_else(|| {
            position_summary
                .and_then(|summary| copy_json_number_or_string(summary, "unrealized_pnl"))
        })
        .map(normalize_display_usd);
    let positions = position_summary
        .and_then(|summary| summary.get("positions"))
        .and_then(Value::as_array)
        .map(|items| {
            items
                .iter()
                .filter_map(copy_pnl_position_summary_from_json)
                .collect::<Vec<_>>()
        })
        .unwrap_or_default();
    Some(CopyPnlAccountSummary {
        ...
        fill_count: fill_summary
            .and_then(|summary| {
                summary
                    .get("fillCount")
                    .or_else(|| summary.get("fill_count"))
            })
        ...
        total_pnl_usd: copy_sum_optional([realized_pnl_usd, unrealized_pnl_usd]),
        fees_usd,
        notional_usd: fill_summary
            .and_then(|summary| copy_json_number_or_string(summary, "notional"))
            .map(normalize_display_usd),
        position_value_usd: position_summary
            .and_then(|summary| copy_json_number_or_string(summary, "positionValue"))
            .or_else(|| {
                position_summary
                    .and_then(|summary| copy_json_number_or_string(summary, "position_value"))
            })
            .map(normalize_display_usd),
        open_position_count: positions.len(),
        positions,
...
fn copy_pnl_position_summary_from_json(value: &Value) -> Option<CopyPnlPositionSummary> {
    let coin = value
        .get("coin")
        .and_then(Value::as_str)
        .unwrap_or("")
        .to_string();
    let signed_size = copy_json_number_or_string(value, "szi")
        .or_else(|| copy_json_number_or_string(value, "size"))
        .unwrap_or(0.0);
    if coin.trim().is_empty() || signed_size.abs() <= 1e-12 {
        return None;
    }
    Some(CopyPnlPositionSummary {
        coin,
        ...
        position_value_usd: copy_json_number_or_string(value, "positionValue")
            .or_else(|| copy_json_number_or_string(value, "position_value"))
            .map(normalize_display_usd),
        unrealized_pnl_usd: copy_json_number_or_string(value, "unrealizedPnl")
            .or_else(|| copy_json_number_or_string(value, "unrealized_pnl"))
            .map(normalize_display_usd),
        entry_px: copy_json_number_or_string(value, "entryPx")
            .or_else(|| copy_json_number_or_string(value, "entry_px")),
    })
}

fn copy_pnl_account_is_local(account: &CopyPnlAccountSummary) -> bool {
    account.kind.eq_ignore_ascii_case("local") || account.id.starts_with("me_")
}

fn copy_pnl_account_is_target(account: &CopyPnlAccountSummary) -> bool {
    account.kind.to_ascii_lowercase().starts_with("target")
        || (!copy_pnl_account_is_local(account) && !account.address.trim().is_empty())
}

fn copy_sum_optional(values: impl IntoIterator<Item = Option<f64>>) -> Option<f64> {
    let mut seen = false;
    let mut total = 0.0;
    ...
    seen.then(|| normalize_display_usd(total))
}

fn copy_json_live_submitted_report(report: &Value) -> bool {
    report
        .get("kind")
        .and_then(Value::as_str)
        .is_some_and(|kind| kind.eq_ignore_ascii_case("submitted"))
        && !report
            .get("dry_run")
...
fn copy_fill_identity(fill: &Value) -> String {
    let hash = fill.get("hash").and_then(Value::as_str).unwrap_or("");
    let oid = fill
        .get("oid")
        .and_then(Value::as_u64)
    ...
    format!("{hash}:{oid}:{time}:{coin}")
}

fn copy_json_number_or_string(value: &Value, key: &str) -> Option<f64> {
...
fn normalize_display_usd(value: f64) -> f64 {
    if value.abs() <= DISPLAY_USD_EPSILON {
        0.0
    } else {
        value
    }
...
fn build_copy_live_soak_status_response() -> Result<CopyLiveSoakStatusResponse> {
    let dir = copy_live_soak_dir();
    build_copy_live_soak_status_response_from_dir(&dir, now_ms())
}

fn copy_live_soak_dir() -> PathBuf {
    let local = PathBuf::from(COPY_LIVE_SOAK_DIR);
    if local.exists() {
        return local;
    }
    ...
    for live in live_accounts {
        ...
        for live_position in &live.positions {
            ...
            if current_value + DISPLAY_USD_EPSILON < live_value
                && copy_pnl_live_value_delta_explained_by_pnl_drift(live_position, current_value)
            {
...
fn copy_pnl_live_value_delta_explained_by_pnl_drift(
    live_position: &CopyPnlPositionSummary,
    owned_position_value_usd: f64,
) -> bool {
    let owned_position_value_usd = owned_position_value_usd.max(0.0);
    let live_position_value_usd = copy_pnl_position_value_usd(live_position);
    if live_position_value_usd <= owned_position_value_usd + DISPLAY_USD_EPSILON {
        return false;
    }
    let delta = live_position_value_usd - owned_position_value_usd;
    let pnl = live_position.unrealized_pnl_usd.unwrap_or_default().abs();
    delta <= pnl + DISPLAY_USD_EPSILON
}
...
fn build_copy_live_soak_status_response_from_dir(
    dir: &Path,
    now_ms_value: u64,
) -> Result<CopyLiveSoakStatusResponse> {
    let copy_settings = read_copy_ui_settings().ok();
    let mut response = CopyLiveSoakStatusResponse {
        detected: false,
        running: false,
        paused: copy_live_soak_pause_flag_exists_in_dir(dir),
        run_id: None,
        pid: None,
        account_ids: copy_settings
            .as_ref()
            .map(|settings| settings.account_ids.clone())
            .unwrap_or_default(),
        markets: copy_settings
            .as_ref()
            .map(|settings| settings.markets.clone())
            .unwrap_or_default(),
        leader_count: copy_settings
            .as_ref()
            .map(|settings| settings.leaders.len())
            .unwrap_or_default(),
    ...
    if !dir.exists() {
        if response.paused {
            ...
            response.status_kind = "paused".to_string();
            response.message =
                "copy live soak is paused by operator; automatic monitor restart is disabled"
                    .to_string();
        }
        return Ok(response);
    }
    ...
    let log_modified_ms = file_modified_at_ms(&log_path).ok();
    let run_id = log_path
        .file_name()
        .and_then(|name| name.to_str())
        .and_then(|name| {
            name.strip_prefix("persistent-live-soak-")
                .and_then(|rest| rest.strip_suffix("-run.log"))
        })
        .unwrap_or("")
        .to_string();
    let last_log_line = last_nonempty_line(&log_path)?;

    response.detected = true;
    response.run_id = if run_id.is_empty() {
        None
    } else {
        Some(run_id.clone())
    };
    ...
    if !run_id.is_empty() {
        ...
        if summary_path.exists() {
            response.latest_summary_path = Some(summary_path.display().to_string());
            if let Some(summary) = read_last_json_line(&summary_path)? {
                response.latest_round = summary.get("round").and_then(Value::as_u64);
                response.latest_ok = summary.get("ok").and_then(Value::as_bool);
                response.submitted_reports =
                    summary.get("submitted_reports").and_then(Value::as_u64);
                response.order_evidence = summary.get("order_evidence").and_then(Value::as_u64);
                response.cleanup_errors = summary.get("cleanup_errors").and_then(Value::as_u64);
                response.final_flat = summary.get("final_flat").and_then(Value::as_bool);
                response.final_reconcile_health = summary
                    .get("final_reconcile_health")
                    .and_then(Value::as_bool);
                response.hold_positions_after_submit = summary
                    .get("hold_positions_after_submit")
                    .and_then(Value::as_bool);
                response.watcher_status = summary
                    .get("watcher_status")
                    .and_then(Value::as_str)
                    .map(str::to_string);
                response.latest_report_path = summary
                    .get("report_path")
                    .and_then(Value::as_str)
                    .map(str::to_string);
                latest_completed_report_path = response.latest_report_path.clone();
            }
        }
    }
    ...
    if let Some(report_path) = response.latest_report_path.as_deref() {
        response.suppression_summaries =
            read_copy_live_soak_suppression_summaries(dir, report_path, 5).unwrap_or_default();
    }
    if response.suppression_summaries.is_empty()
        && let Some(report_path) = latest_completed_report_path.as_deref()
        && response.latest_report_path.as_deref() != Some(report_path)
    {
        response.suppression_summaries =
            read_copy_live_soak_suppression_summaries(dir, report_path, 5).unwrap_or_default();
    }
    if response.suppression_summaries.is_empty() && !run_id.is_empty() {
        response.suppression_summaries =
            read_latest_copy_live_soak_suppression_summaries(dir, &run_id, 5).unwrap_or_default();
    }
    ...
    let effective_stale = !response.paused && stale && !pid_running;
    response.stale = effective_stale;
    response.pid_running = pid_running;
    response.terminal = !response.paused && (terminal || active_report_stalled);
    response.running = !response.paused && !effective_stale && !response.terminal && pid_running;
    ...
    response.message = if response.running {
        ...
        "copy live soak is paused by operator; automatic monitor restart is disabled".to_string()
    } else if active_report_stalled {
...
fn read_copy_live_soak_suppression_summaries(
    dir: &Path,
    report_path: &str,
    limit: usize,
) -> Result<Vec<CopyLiveSoakSuppressionSummary>> {
    if limit == 0 {
        return Ok(Vec::new());
    }
    ...
    let path = if path.is_absolute() {
        ...
        if direct.exists() {
            ...
            let file_name = path
                .file_name()
                .with_context(|| format!("invalid live soak report path {report_path}"))?;
            dir.join(file_name)
        }
    };
    if !path.exists() || fs::metadata(&path).map(|meta| meta.len()).unwrap_or(0) == 0 {
        return Ok(Vec::new());
    }
    ...
    let submitted_cloids = copy_live_soak_submitted_cloids(&value);
    let mut summaries = Vec::<CopyLiveSoakSuppressionSummary>::new();
    let items = value
        .get("suppressed_would_submit_orders")
        .and_then(Value::as_array)
        .cloned()
        .or_else(|| {
            value
                .get("suppressed_submit_plan_refs")
                .and_then(Value::as_array)
                .cloned()
        })
        .unwrap_or_default();
    for item in items {
        ...
        let order = item
            .get("order")
            .or_else(|| item.get("plan").and_then(|plan| plan.get("order")));
        let cloid = order
            .and_then(|order| order.get("cloid"))
            .and_then(Value::as_str)
            .map(normalize_copy_soak_cloid_key);
        if cloid
            .as_deref()
            .is_some_and(|cloid| submitted_cloids.contains(cloid))
        {
            continue;
        }
        let account_id = order
            .and_then(|order| order.get("account_id"))
            .and_then(Value::as_str)
            .map(str::to_string);
        let coin = order
            ...
            .and_then(Value::as_str)
            .map(str::to_string);
        if let Some(existing) = summaries.iter_mut().find(|summary| {
        ...
        summaries.push(CopyLiveSoakSuppressionSummary {
            reason_code,
            message,
            account_id,
            coin,
...
fn read_latest_copy_live_soak_suppression_summaries(
    dir: &Path,
    run_id: &str,
    limit: usize,
) -> Result<Vec<CopyLiveSoakSuppressionSummary>> {
    if limit == 0 || run_id.trim().is_empty() || !dir.exists() {
        return Ok(Vec::new());
    }
    let mut reports = Vec::<PathBuf>::new();
    for entry in fs::read_dir(dir).with_context(|| format!("failed to read {}", dir.display()))? {
        let path = entry?.path();
        if copy_live_soak_round_report_path_for_run(&path, Some(run_id)) {
            reports.push(path);
        }
    }
    ...
    for report in reports {
        let summaries =
            read_copy_live_soak_suppression_summaries(dir, &report.display().to_string(), limit)?;
...
fn copy_live_soak_submitted_cloids(value: &Value) -> HashSet<String> {
    value
        .get("persistent_live_submit")
        .and_then(|submit| submit.get("submitted_reports"))
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .filter(|report| {
        ...
        .filter_map(|report| report.get("cloid").and_then(Value::as_str))
        .map(normalize_copy_soak_cloid_key)
        .filter(|cloid| !cloid.is_empty())
        .collect()
}

fn normalize_copy_soak_cloid_key(cloid: &str) -> String {
    cloid
        .trim()
...
fn copy_live_soak_pid_file(dir: &Path) -> Result<Option<PathBuf>> {
    let current = dir.join("persistent-live-soak-detached-current.pid");
    if current.exists() {
    ...
    for entry in fs::read_dir(dir).with_context(|| format!("failed to read {}", dir.display()))? {
        ...
        let name = path
            ...
            .and_then(|value| value.to_str())
            .unwrap_or("");
        if !(name.starts_with("persistent-live-soak") || name.starts_with("persistent-follow-soak"))
            || !name.ends_with(".pid")
        {
            continue;
        }
...
fn last_nonempty_line(path: &Path) -> Result<String> {
    let raw = fs::read_to_string(path)
        .with_context(|| format!("failed to read live soak log {}", path.display()))?;
    Ok(raw
        .lines()
        .rev()
        .map(str::trim)
...
fn read_last_json_line(path: &Path) -> Result<Option<Value>> {
    let raw = fs::read_to_string(path)
    ...
    for line in raw.lines().rev() {
        ...
        return serde_json::from_str::<Value>(line)
            .map(Some)
            .with_context(|| format!("failed to parse live soak summary {}", path.display()));
    }
    Ok(None)
}
...
struct LiveReadinessPlanResponse {
    ...
    limit_price: f64,
    size: f64,
    effective_notional_usd: f64,
    minimum_requested_notional_usd: Option<f64>,
    sz_decimals: u32,
    execution_mode: ExecutionMode,
    tif: String,
}
    ...
    let plan = match plan_result {
        Ok(plan) => {
            ...
            checks.push(ReadinessCheckResponse::blocker(
                ...
                exchange_min_effective_detail(effective_notional, minimum_requested_notional),
            ));
...
struct FibInstancesResponse {
    instances: Vec<FibInstanceResponse>,
    fetched_at_ms: u64,
}

#[derive(Debug, Clone, Serialize)]
struct FibInstanceResponse {
    #[serde(flatten)]
    record: FibInstanceRecord,
    pnl_summary: FibStrategyPnlSummary,
}
...
#[derive(Debug, Serialize)]
struct CopyShadowHistoryResponse {
    entries: Vec<CopyShadowHistoryEntry>,
    fetched_at_ms: u64,
}

#[derive(Debug, Serialize)]
struct CopyLiveSoakStatusResponse {
    detected: bool,
    running: bool,
    paused: bool,
    ...
    latest_ok: Option<bool>,
    submitted_reports: Option<u64>,
    order_evidence: Option<u64>,
    cleanup_errors: Option<u64>,
    suppression_summaries: Vec<CopyLiveSoakSuppressionSummary>,
    final_flat: Option<bool>,
...
struct CopyLiveSoakSuppressionSummary {
    reason_code: String,
    message: String,
    account_id: Option<String>,
    coin: Option<String>,
...
fn last_matching_line(path: &Path, needle: &str) -> Result<Option<String>> {
    let text =
        fs::read_to_string(path).with_context(|| format!("failed to read {}", path.display()))?;
    Ok(text
        .lines()
...
fn parse_copy_soak_log_csv_field(line: &str, field: &str) -> Option<Vec<String>> {
    let value = parse_copy_soak_log_field(line, field)?;
    let values = value
        .split(',')
        .map(str::trim)
        .filter(|token| !token.is_empty())
...
fn parse_copy_soak_log_usize_field(line: &str, field: &str) -> Option<usize> {
    parse_copy_soak_log_field(line, field)?
        .parse::<usize>()
        .ok()
}

fn parse_copy_soak_log_field(line: &str, field: &str) -> Option<String> {
...
impl CopySettingsPayload {
    fn normalized(self) -> Result<CopyUiSettings> {
        ...
        let mut account_ids = normalize_copy_account_ids(String::new(), self.account_ids);
        let account_id = self
            .account_id
            .map(|account_id| account_id.trim().to_string())
            .filter(|account_id| !account_id.is_empty())
            .or_else(|| account_ids.first().cloned())
            .unwrap_or_else(|| "addr_a".to_string());
        anyhow::ensure!(!account_id.is_empty(), "account_id cannot be empty");
        account_ids = normalize_copy_account_ids(account_id.clone(), account_ids);
        Ok(CopyUiSettings {
            leaders,
            markets,
...
struct CopySummaryResponse {
    ...
    target_total_pnl_usd: Option<f64>,
    target_position_value_usd: Option<f64>,
    target_open_position_count: u64,
    target_position_state: String,
    latest_entries: Vec<CopyShadowHistoryEntry>,
}
...
struct SmartMoneyPreviewPayload {
    ...
    copy_ratio: f64,
    #[serde(default = "default_copy_principal_cap_usd")]
    principal_cap_usd: f64,
    #[serde(default = "default_copy_leverage")]
    leverage: f64,
    #[serde(default)]
    max_signal_notional_usd: Option<f64>,
    reduce_only: bool,
    #[serde(default)]
    market: Option<String>,
}
...
impl SmartMoneyPreviewPayload {
    ...
    fn sizing_preview(&self) -> Result<SmartMoneySizingPreview> {
        anyhow::ensure!(
            self.leader_notional_usd > 0.0,
            "leader notional must be positive"
        );
        anyhow::ensure!(self.copy_ratio > 0.0, "copy ratio must be positive");
        anyhow::ensure!(
            self.copy_ratio <= 1.0,
            "copy ratio must be less than or equal to 1"
        );
        anyhow::ensure!(
            self.principal_cap_usd > 0.0,
            "principal cap must be positive"
        );
        anyhow::ensure!(self.leverage >= 1.0, "leverage must be at least 1x");
        anyhow::ensure!(
            self.leverage <= crate::strategies::smart_money::COPY_MAX_LEVERAGE,
            "leverage must be less than or equal to {}",
            crate::strategies::smart_money::COPY_MAX_LEVERAGE
        );

        let raw_principal_usd = self.leader_notional_usd * self.copy_ratio;
        anyhow::ensure!(
            raw_principal_usd.is_finite() && raw_principal_usd > 0.0,
            "copy principal must be positive"
        );
        let capped_principal_usd = raw_principal_usd.min(self.principal_cap_usd);
        let leveraged_notional_usd = capped_principal_usd * self.leverage;
        anyhow::ensure!(
            leveraged_notional_usd.is_finite() && leveraged_notional_usd > 0.0,
            "leveraged notional must be positive"
        );

        Ok(SmartMoneySizingPreview {
            leader_notional_usd: self.leader_notional_usd,
            copy_ratio: self.copy_ratio,
            raw_principal_usd,
            principal_cap_usd: self.principal_cap_usd,
            capped_principal_usd,
            capped_by_principal: raw_principal_usd > self.principal_cap_usd,
            leverage: self.leverage,
            leveraged_notional_usd,
            max_leverage: crate::strategies::smart_money::COPY_MAX_LEVERAGE,
        })
    }
    ...
    fn try_into_signal(
        self,
        config: &AppConfig,
        sizing: &SmartMoneySizingPreview,
    ) -> Result<Vec<crate::domain::CoordinatorSignal>> {
        let mut strategy = SmartMoneyCopyStrategy::new(SmartMoneyCopyConfig {
            strategy_id: "smart_money_preview".to_string(),
            default_copy_ratio: self.copy_ratio * sizing.leverage,
            max_slippage_bps: 25.0,
            leaders: vec![LeaderRule {
                leader_id: self.leader_id.clone(),
                leader_address: self.leader_address.clone(),
                enabled: true,
                copy_ratio: self.copy_ratio * sizing.leverage,
            }],
            symbol_limits: vec![SymbolCopyLimit {
                coin: self.coin.clone(),
                max_signal_notional_usd: sizing.principal_cap_usd * sizing.leverage,
            }],
        });
...
#[derive(Debug, Clone, Serialize)]
struct SmartMoneySizingPreview {
    leader_notional_usd: f64,
    copy_ratio: f64,
    raw_principal_usd: f64,
    principal_cap_usd: f64,
    capped_principal_usd: f64,
    capped_by_principal: bool,
    leverage: f64,
    leveraged_notional_usd: f64,
    max_leverage: f64,
}

fn default_copy_principal_cap_usd() -> f64 {
    crate::strategies::smart_money::COPY_DEFAULT_PRINCIPAL_CAP_USD
}

fn default_copy_leverage() -> f64 {
    crate::strategies::smart_money::COPY_MAX_LEVERAGE
}
...
fn resolve_market_profile(raw_market: Option<&str>, config: &AppConfig) -> Result<MarketProfile> {
    ...
    let profile = match market_id {
        ...
        MARKET_CASH_PERP => MarketProfile {
            id: MARKET_CASH_PERP,
            label: "Cash Perps",
            dex: "cash".to_string(),
            live_trading_supported: true,
...
mod tests {
    ...
    #[test]
    fn copy_shadow_history_response_reads_recent_entries() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_shadow_history_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        let path = dir.join("copy_shadow_history.jsonl");
        let first = crate::strategies::smart_money::CopyShadowHistoryEntry {
            schema_version: 1,
            occurred_at_ms: 100,
            ...
            action_event_id: "evt_a".to_string(),
            live_gate: "dry_run_only".to_string(),
            risk_reject_reason: None,
            signal_id: Some("sig_a".to_string()),
        ...
        let second = crate::strategies::smart_money::CopyShadowHistoryEntry {
            occurred_at_ms: 200,
            action_event_id: "evt_b".to_string(),
            signal_id: Some("sig_b".to_string()),
            ..first.clone()
        };
        crate::strategies::smart_money::append_copy_shadow_history_entry(&path, &first)
            .expect("append first copy shadow entry");
        crate::strategies::smart_money::append_copy_shadow_history_entry(&path, &second)
            .expect("append second copy shadow entry");

        let response =
            build_copy_shadow_history_response_from_path(&path, 1).expect("shadow response");

        assert_eq!(response.entries.len(), 1);
        assert_eq!(response.entries[0].action_event_id, "evt_b");
        assert_eq!(response.entries[0].occurred_at_ms, 200);
        assert!(response.fetched_at_ms > 0);
    }

    #[test]
    fn copy_summary_counts_user_friendly_metrics() {
        let copied = crate::strategies::smart_money::CopyShadowHistoryEntry {
            schema_version: 1,
            occurred_at_ms: 100,
            status: "would_copy".to_string(),
            leader_id: "leader_a".to_string(),
        ...
        let deduped = crate::strategies::smart_money::CopyShadowHistoryEntry {
            status: "deduped".to_string(),
            leader_address: "0xdef".to_string(),
            occurred_at_ms: 300,
            action_event_id: "evt_c".to_string(),
            notional_usd: None,
            ..copied.clone()
        };

        let summary = build_copy_summary_response_from_entries(
            vec![copied, rejected, deduped],
            None,
            400,
    ...
    #[test]
    fn copy_summary_includes_pnl_report_accounts() {
        let copied = crate::strategies::smart_money::CopyShadowHistoryEntry {
            schema_version: 1,
            occurred_at_ms: 100,
            status: "would_copy".to_string(),
            leader_id: "swing_1".to_string(),
        ...
        let pnl_report = CopyPnlReportSummary {
            ...
            accounts: vec![
                CopyPnlAccountSummary {
                    ...
                    total_pnl_usd: Some(-0.20),
                    fees_usd: Some(0.02),
                    ..CopyPnlAccountSummary::default()
                },
                CopyPnlAccountSummary {
                    id: "swing_1".to_string(),
                    kind: "target_swing".to_string(),
                    address: "0x9dead8fffcbf130e7658f672d2c081d91178d617".to_string(),
                    fill_count: 3,
                    realized_pnl_usd: Some(1.50),
                    unrealized_pnl_usd: Some(-0.25),
        ...
        assert_eq!(summary.leader_summaries[0].signal_count, 1);
        assert_eq!(summary.target_realized_pnl_usd, Some(1.5));
        assert_eq!(summary.target_unrealized_pnl_usd, Some(-0.25));
        assert_eq!(summary.target_total_pnl_usd, Some(1.25));
        assert_eq!(summary.target_open_position_count, 1);
        assert_eq!(
            summary
                .local_summary
                .as_ref()
                .and_then(|item| item.total_pnl_usd),
            Some(-0.20)
        );
        assert_eq!(
    ...
    fn copy_settings_payload_normalizes_multiple_markets() -> Result<()> {
        ...
        assert_eq!(
            ...
            vec![
                ...
                MARKET_CASH_PERP.to_string(),
                MARKET_SPOT.to_string()
            ]
        );
        assert_eq!(settings.leaders.len(), 2);
        assert_eq!(settings.leverage, 7.0);
    ...
    #[test]
    fn copy_settings_apply_updates_selected_multiple_accounts_only() -> Result<()> {
        let settings = super::CopySettingsPayload {
            leaders: vec!["0x6d6d7c05ef7f31b31b618400495b4ce4092a5089".to_string()],
            markets: vec![MARKET_XYZ_PERP.to_string(), MARKET_HL_PERP.to_string()],
            copy_ratio: 0.25,
            principal_cap_usd: 20.0,
            leverage: Some(6.0),
            account_id: Some("addr_a".to_string()),
            account_ids: vec![
                "addr_a".to_string(),
        ...
        for account_id in ["addr_a", "addr_b"] {
            let account = config.account(account_id).expect("selected account");
            assert_eq!(account.copy_ratio, 0.25);
            assert_eq!(account.max_order_notional_usd, 120.0);
        }
        let untouched = config.account("addr_c").expect("unselected account");
        assert_eq!(untouched.copy_ratio, 1.0);
        assert_eq!(untouched.max_order_notional_usd, 100.0);
        Ok(())
    }

    #[test]
    fn copy_settings_payload_normalizes_multiple_local_accounts() -> Result<()> {
        let settings = super::CopySettingsPayload {
            leaders: vec!["0x6d6d7c05ef7f31b31b618400495b4ce4092a5089".to_string()],
            markets: vec![MARKET_XYZ_PERP.to_string()],
            copy_ratio: 0.2,
            principal_cap_usd: 35.0,
            leverage: None,
            account_id: Some("addr_a".to_string()),
            account_ids: vec!["addr_b, addr_a".to_string(), "addr_c".to_string()],
        }
        .normalized()?;
    ...
    #[test]
    fn copy_settings_payload_uses_account_ids_when_primary_missing() -> Result<()> {
        let settings = super::CopySettingsPayload {
            leaders: vec!["0x6d6d7c05ef7f31b31b618400495b4ce4092a5089".to_string()],
            markets: vec![MARKET_XYZ_PERP.to_string()],
            copy_ratio: 0.2,
            ...
            leverage: Some(3.0),
            account_id: None,
            account_ids: vec!["addr_b".to_string()],
        }
        .normalized()?;

        assert_eq!(settings.account_id, "addr_b");
        assert_eq!(settings.account_ids, vec!["addr_b".to_string()]);
    ...
    #[test]
    fn copy_summary_aggregates_multiple_local_live_accounts() {
        let configured = CopyUiSettings {
            leaders: default_copy_leaders(),
            markets: default_copy_markets(),
            copy_ratio: 0.2,
        ...
        let local_b = CopyPnlAccountSummary {
            id: "addr_b".to_string(),
            kind: "local_live".to_string(),
            unrealized_pnl_usd: Some(-0.5),
            total_pnl_usd: Some(-0.5),
            position_value_usd: Some(50.0),
            open_position_count: 1,
            positions: vec![CopyPnlPositionSummary {
                coin: "xyz:MU".to_string(),
                side: "short".to_string(),
                ...
                position_value_usd: Some(50.0),
                unrealized_pnl_usd: Some(-0.5),
                entry_px: Some(1000.0),
            }],
            ..CopyPnlAccountSummary::default()
        };
        let live_accounts = CopyLiveAccountSummaries {
            local: aggregate_copy_local_account_summaries(&[local_a.clone(), local_b.clone()]),
            locals: vec![local_a, local_b],
            targets: Vec::new(),
        };
        ...
        assert_eq!(summary.local_summaries.len(), 2);
        assert_eq!(summary.unrealized_pnl_usd, Some(0.75));
        assert_eq!(
            summary
                .local_summary
                .as_ref()
                .and_then(|item| item.position_value_usd),
            Some(150.0)
        );
        assert_eq!(
            summary
    ...
    #[test]
    fn copy_summary_treats_copy_mark_to_market_drift_as_copy_owned() {
        let configured = CopyUiSettings {
            leaders: default_copy_leaders(),
            markets: default_copy_markets(),
            copy_ratio: 0.2,
        ...
        let live_position = CopyPnlPositionSummary {
            ...
            side: "short".to_string(),
            size: 22.0,
            position_value_usd: Some(193.3052),
            unrealized_pnl_usd: Some(-3.2252),
            entry_px: Some(8.64),
        };
        let local_a = CopyPnlAccountSummary {
            id: "addr_a".to_string(),
        ...
        let ledger_entries = vec![crate::strategies::smart_money::CopyLedgerEntry {
            ...
            leader_id: "leader_4".to_string(),
            leader_group: "leader_4".to_string(),
            signal_id: "copy-leader_4-open-purrdat".to_string(),
            coin: "xyz:PURRDAT".to_string(),
            local_side: OrderSide::Sell,
            order_cloid: Some("cloid-open-purrdat".to_string()),
            order_oid: Some(481_528_452_417),
            submitted_at_ms: Some(100),
            filled_at_ms: Some(101),
            planned_notional_usd: 190.08,
            pending_notional_usd: 0.0,
            filled_notional_usd: 190.08,
            remaining_notional_usd: 190.08,
            status: crate::strategies::smart_money::CopyLedgerStatus::Open,
        }];
        ...
        let copy_owned = summary.copy_owned_summary.expect("copy owned summary");
        assert_eq!(copy_owned.open_position_count, 1);
        assert_eq!(copy_owned.position_value_usd, Some(193.3052));
        assert_eq!(copy_owned.unrealized_pnl_usd, Some(-3.2252));
        assert!(
            summary
                .other_local_summary
                .as_ref()
    ...
    #[test]
    fn copy_summary_keeps_flat_local_account_rows() {
        let configured = CopyUiSettings {
            leaders: default_copy_leaders(),
            markets: default_copy_markets(),
            copy_ratio: 0.2,
        ...
        assert_eq!(summary.local_summaries.len(), 2);
        assert_eq!(summary.local_summaries[1].id, "addr_b");
        assert_eq!(summary.local_summaries[1].open_position_count, 0);
        assert_eq!(summary.unrealized_pnl_usd, Some(2.0));
    }
    ...
    #[test]
    fn copy_live_soak_pnl_summary_reads_round_fill_evidence() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_pnl_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        let report_path = dir.join("persistent-live-soak-20260617-010203-round-0001.json");
        fs::write(
            &report_path,
            serde_json::json!({
                "persistent_live_submit": {
                    "submitted_reports": [
                        {"kind": "submitted", "dry_run": false},
                        {"kind": "error", "message": "Insufficient margin to place order"},
                        {"kind": "submitted", "dry_run": true}
                    ],
                    "order_evidence": [{
                        "matching_fills": [{
                            "coin": "xyz:JPY",
                            "oid": 1001,
                            "time": 1781565821472_u64,
                            "hash": "0xabc",
                            "closedPnl": "1.25",
                            "fee": "0.05"
                        }, {
    ...
    #[test]
    fn copy_shadow_history_merges_active_paths_by_recency() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_shadow_history_merge_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        let default_path = dir.join("copy_shadow_history.jsonl");
        let active_path = dir.join("active-shadow.jsonl");
        let first = crate::strategies::smart_money::CopyShadowHistoryEntry {
            schema_version: 1,
            occurred_at_ms: 100,
            status: "would_copy".to_string(),
        ...
        crate::strategies::smart_money::append_copy_shadow_history_entry(&default_path, &first)
            .expect("append default copy shadow entry");
        crate::strategies::smart_money::append_copy_shadow_history_entry(&active_path, &second)
            .expect("append active copy shadow entry");

        let entries =
            read_recent_copy_shadow_history_entries_from_paths(vec![default_path, active_path], 10)
                .expect("merged shadow entries");

        assert_eq!(entries.len(), 2);
        assert_eq!(entries[0].action_event_id, "evt_b");
        assert_eq!(entries[1].action_event_id, "evt_a");
    }

    #[test]
    fn smart_money_preview_caps_principal_before_leverage() {
        let payload = smart_money_preview_payload(1000.0, 0.1, 10.0, 5.0);

        let sizing = payload.sizing_preview().expect("sizing preview");
        let signals = payload
            .try_into_signal(&AppConfig::default(), &sizing)
            .expect("copy signal");

        assert_eq!(signals.len(), 1);
        assert_eq!(sizing.raw_principal_usd, 100.0);
        assert_eq!(sizing.capped_principal_usd, 10.0);
        assert!(sizing.capped_by_principal);
        assert_eq!(signals[0].order.notional_usd, 50.0);
    }

    #[test]
    fn smart_money_preview_keeps_below_cap_aligned_to_ratio() {
        let payload = smart_money_preview_payload(50.0, 0.1, 10.0, 5.0);
        ...
        assert_eq!(signals.len(), 1);
        assert_eq!(sizing.raw_principal_usd, 5.0);
        assert_eq!(sizing.capped_principal_usd, 5.0);
        assert!(!sizing.capped_by_principal);
        assert_eq!(signals[0].order.notional_usd, 25.0);
    }
    ...
    #[test]
    fn smart_money_preview_rejects_leverage_below_one() {
        let payload = smart_money_preview_payload(50.0, 0.1, 10.0, 0.5);

        let error = payload
            .sizing_preview()
            .expect_err("leverage below 1x must be rejected")
            .to_string();
    ...
    #[test]
    fn copy_live_soak_status_reads_latest_running_summary() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_running_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        fs::write(
            dir.join("persistent-live-soak-20260615-214919-run.log"),
            "2026-06-15T22:31:15+08:00 round=5 starting report=.codex-longrun\\persistent-live-soak-20260615-214919-round-0005.json\n",
        )
        .expect("run log");
        fs::write(
            dir.join("persistent-live-soak-20260615-214919-summary.jsonl"),
            "{\"run_id\":\"20260615-214919\",\"round\":4,\"ok\":true,\"submitted_reports\":0,\"order_evidence\":1,\"cleanup_errors\":0,\"watcher_status\":\"completed_duration_watcher_open\",\"final_flat\":false,\"final_reconcile_health\":true,\"hold_positions_after_submit\":true,\"report_path\":\".codex-longrun\\\\persistent-live-soak-20260615-214919-round-0004.json\"}\n",
        )
        .expect("summary");
        fs::write(
            dir.join("persistent-live-soak-detached-current.pid"),
        ...
        .expect("pid");
        fs::write(dir.join("frontend-current.pid"), "99999\n").expect("frontend pid");
    ...
    #[test]
    fn copy_live_soak_status_reads_runtime_accounts_and_markets_from_log() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_runtime_scope_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        fs::write(
            dir.join("persistent-live-soak-20260623-203800-run.log"),
            "2026-06-23T20:38:01+08:00 copy settings leaders=5 accounts=addr_a,addr_b markets=xyz_perp,hl_perp,spot leader_notional_usd=1750\n2026-06-23T20:38:01+08:00 round=1 starting report=.codex-longrun\\persistent-live-soak-20260623-203800-round-0001.json\n",
        )
        .expect("run log");
        fs::write(
            dir.join("persistent-live-soak-detached-current.pid"),
        ...
        assert_eq!(status.leader_count, 5);
    }

    #[test]
    fn copy_live_soak_status_paused_keeps_current_settings_scope() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_paused_scope_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        fs::write(
            dir.join("persistent-live-soak-20260623-224946-run.log"),
            "2026-06-23T22:49:46+08:00 copy settings leaders=5 accounts=addr_a markets=xyz_perp leader_notional_usd=1750\n",
        )
        .expect("run log");
        fs::create_dir_all(COPY_LIVE_SOAK_DIR).expect("live dir");
        let settings_path = Path::new(COPY_UI_SETTINGS_PATH);
        let previous_settings = fs::read(settings_path).ok();
        fs::write(
            settings_path,
            serde_json::to_vec(&CopyUiSettings {
                leaders: default_copy_leaders(),
                markets: vec![MARKET_XYZ_PERP.to_string(), MARKET_HL_PERP.to_string()],
                copy_ratio: 0.2,
                principal_cap_usd: 35.0,
        ...
        fs::write(dir.join("copy-live-soak-paused.flag"), "{}\n").expect("pause flag");

        let status = build_copy_live_soak_status_response_from_dir(&dir, crate::domain::now_ms())
            .expect("status");
        let _ = fs::remove_file(dir.join("copy-live-soak-paused.flag"));
        if let Some(previous_settings) = previous_settings {
            fs::write(settings_path, previous_settings).expect("restore settings");
        } else {
            let _ = fs::remove_file(settings_path);
        }

        assert!(status.paused);
        assert_eq!(status.status_kind, "paused");
        assert_eq!(status.run_id.as_deref(), Some("20260623-224946"));
        assert_eq!(
            status.account_ids,
            vec!["addr_a".to_string(), "addr_b".to_string()]
        );
    ...
    #[test]
    fn copy_live_soak_status_paused_keeps_latest_run_evidence_and_plan_suppressions() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_paused_evidence_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        fs::write(
            dir.join("persistent-live-soak-20260624-045617-run.log"),
            "2026-06-24T04:56:17+08:00 copy settings leaders=5 accounts=addr_a,addr_b markets=xyz_perp,hl_perp,cash_perp,spot leader_notional_usd=1750\n2026-06-24T05:06:24+08:00 round=1 ok=True submitted=1 evidence=1\n2026-06-24T05:06:29+08:00 round=2 starting report=.codex-longrun\\persistent-live-soak-20260624-045617-round-0002.json\n",
        )
        .expect("run log");
        fs::write(
            dir.join("persistent-live-soak-20260624-045617-summary.jsonl"),
            "{\"run_id\":\"20260624-045617\",\"round\":1,\"ok\":true,\"submitted_reports\":1,\"order_evidence\":1,\"cleanup_errors\":0,\"watcher_status\":\"completed_duration_watcher_open\",\"final_flat\":false,\"final_reconcile_health\":true,\"hold_positions_after_submit\":true,\"report_path\":\"persistent-live-soak-20260624-045617-round-0001.json\"}\n",
        )
        .expect("summary");
        fs::write(
            dir.join("persistent-live-soak-20260624-045617-round-0001.json"),
            serde_json::json!({
                "persistent_live_submit": {
                    "submitted_reports": [{"kind": "submitted", "dry_run": false, "notional_usd": 12.5}],
                    "order_evidence": [{"matching_fills": []}]
                },
                "suppressed_submit_plan_refs": [{
                    ...
                    "reason_code": "COPY_DAEMON_MARGIN_RESIZED_BELOW_MIN",
                    "message": "addr_a withdrawable=0.2 supports resized_notional=2.0, below exchange minimum 10"
                }]
            })
            .to_string(),
        )
        ...
        fs::write(
            ...
            serde_json::json!({
                "persistent_live_submit": {
                    "submitted_reports": [{"kind": "submitted", "dry_run": false, "notional_usd": 999.0}],
                    "order_evidence": [{"matching_fills": []}]
                }
            })
            .to_string(),
        ...
        let status = build_copy_live_soak_status_response_from_dir(&dir, crate::domain::now_ms())
            .expect("status");
        let pnl_summary =
            read_copy_live_soak_pnl_summary(&dir, status.run_id.as_deref()).expect("pnl summary");

        assert!(status.paused);
        assert_eq!(status.status_kind, "paused");
        assert_eq!(status.run_id.as_deref(), Some("20260624-045617"));
        assert_eq!(status.latest_round, Some(1));
        assert_eq!(status.submitted_reports, Some(1));
        assert_eq!(status.order_evidence, Some(1));
        assert_eq!(status.suppression_summaries.len(), 1);
        assert_eq!(
            status.suppression_summaries[0].reason_code,
            "COPY_DAEMON_MARGIN_RESIZED_BELOW_MIN"
        );
        assert_eq!(
            status.suppression_summaries[0].account_id.as_deref(),
            Some("addr_a")
        );
        assert_eq!(
            status.suppression_summaries[0].coin.as_deref(),
            Some("xyz:JPY")
        );
        assert_eq!(pnl_summary.submitted_reports, 1);
        assert_eq!(pnl_summary.order_evidence, 1);
        assert!((pnl_summary.submitted_notional_usd - 12.5).abs() < 1e-9);
    }
    ...
    #[test]
    fn copy_live_soak_status_marks_stale_log_not_running() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_stale_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        fs::write(
        ...
        .expect("run log");
        let stale_now = crate::domain::now_ms() + 26 * 60 * 1000;

        let status =
            build_copy_live_soak_status_response_from_dir(&dir, stale_now).expect("status");

        assert!(status.detected);
        assert!(!status.running);
        assert_eq!(status.message, "latest live soak log is stale");
    }

    #[test]
    fn copy_live_soak_status_marks_dead_pid_not_running() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_dead_pid_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        fs::write(
    ...
    #[test]
    fn copy_live_soak_status_keeps_recent_suppression_during_empty_active_round() {
        let dir = std::env::temp_dir().join(format!(
            "trade_xyz_copy_live_soak_recent_suppression_{}",
            crate::domain::now_ms()
        ));
        fs::create_dir_all(&dir).expect("test dir");
        fs::write(
            dir.join("persistent-live-soak-20260624-063702-run.log"),
            "2026-06-24T06:37:02+08:00 copy settings leaders=5 accounts=addr_a,addr_b markets=xyz_perp,hl_perp,cash_perp,spot leader_notional_usd=1750\n2026-06-24T06:47:11+08:00 round=1 ok=True submitted=0 evidence=0\n2026-06-24T06:57:25+08:00 round=2 ok=True submitted=0 evidence=0\n2026-06-24T06:57:30+08:00 round=3 starting report=.codex-longrun\\persistent-live-soak-20260624-063702-round-0003.json\n",
        )
        .expect("run log");
        fs::write(
            dir.join("persistent-live-soak-20260624-063702-summary.jsonl"),
            "{\"run_id\":\"20260624-063702\",\"round\":1,\"ok\":true,\"submitted_reports\":0,\"order_evidence\":0,\"cleanup_errors\":0,\"watcher_status\":\"completed_duration_watcher_open\",\"final_flat\":false,\"final_reconcile_health\":true,\"hold_positions_after_submit\":true,\"report_path\":\"persistent-live-soak-20260624-063702-round-0001.json\"}\n{\"run_id\":\"20260624-063702\",\"round\":2,\"ok\":true,\"submitted_reports\":0,\"order_evidence\":0,\"cleanup_errors\":0,\"watcher_status\":\"completed_duration_watcher_open\",\"final_flat\":false,\"final_reconcile_health\":true,\"hold_positions_after_submit\":true,\"report_path\":\"persistent-live-soak-20260624-063702-round-0002.json\"}\n",
        )
        .expect("summary");
        fs::write(
            dir.join("persistent-live-soak-20260624-063702-round-0001.json"),
            serde_json::json!({
                "persistent_live_submit": {
                    "submitted_reports": [],
                    "order_evidence": []
                ...
                "suppressed_submit_plan_refs": [{
                    ...
                    "reason_code": "COPY_DAEMON_MARGIN_RESIZED_BELOW_MIN",
                    "message": "addr_b withdrawable=0.292874 supports resized_notional=2.638505, below exchange minimum 10.000000"
                }]
            })
            .to_string(),
        )
        .expect("round one report");
        fs::write(
            dir.join("persistent-live-soak-20260624-063702-round-0002.json"),
            serde_json::json!({
                "persistent_live_submit": {
                    "submitted_reports": [],
                    "order_evidence": []
        ...
        .expect("round two report");
        fs::write(
            dir.join("persistent-live-soak-20260624-063702-round-0003.json"),
            "",
        )
        .expect("active report");
        fs::write(
        ...
        assert!(status.running);
        assert_eq!(status.latest_round, Some(3));
        assert_eq!(status.suppression_summaries.len(), 1);
        assert_eq!(
            status.suppression_summaries[0].reason_code,
            "COPY_DAEMON_MARGIN_RESIZED_BELOW_MIN"
    ...
    fn smart_money_preview_payload(
        ...
        principal_cap_usd: f64,
        leverage: f64,
    ) -> super::SmartMoneyPreviewPayload {
        super::SmartMoneyPreviewPayload {
            leader_id: "leader_a".to_string(),
            leader_address: "0xabc".to_string(),
            ...
            principal_cap_usd,
            leverage,
            max_signal_notional_usd: None,
            reduce_only: false,
            market: Some(MARKET_XYZ_PERP.to_string()),
        }
    }
